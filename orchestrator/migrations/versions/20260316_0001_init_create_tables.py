...
def upgrade() -> None:
    ...
    op.create_index("ix_agents_account_id", "agents", ["account_id"], schema=SCHEMA)
    ...
    op.create_index("ix_connections_account_id", "connections", ["account_id"], schema=SCHEMA)

    op.create_table(
        "pipelines",
        sa.Column("pipeline_id", UUID(as_uuid=True), primary_key=True),
        sa.Column("account_id", UUID(as_uuid=True), nullable=False),
        sa.Column("agent_id", UUID(as_uuid=True), sa.ForeignKey(f"{SCHEMA}.agents.agent_id"), nullable=False),
        sa.Column(
            "connection_id", UUID(as_uuid=True), sa.ForeignKey(f"{SCHEMA}.connections.connection_id"), nullable=False
        ),
    ...
    op.create_index("ix_pipelines_account_id", "pipelines", ["account_id"], schema=SCHEMA)

    op.create_table(
        "job_runs",
        sa.Column("job_id", UUID(as_uuid=True), primary_key=True),
        sa.Column("pipeline_id", UUID(as_uuid=True), sa.ForeignKey(f"{SCHEMA}.pipelines.pipeline_id"), nullable=False),
        sa.Column("agent_id", UUID(as_uuid=True), sa.ForeignKey(f"{SCHEMA}.agents.agent_id"), nullable=False),
    ...
    op.create_index("ix_job_runs_pipeline_id", "job_runs", ["pipeline_id"], schema=SCHEMA)
    op.create_index("ix_job_runs_agent_id", "job_runs", ["agent_id"], schema=SCHEMA)
